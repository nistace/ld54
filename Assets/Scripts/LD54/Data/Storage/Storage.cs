...
namespace LD54.Data {
	public class Storage : MonoBehaviour {
		...
		public Package.Event onPackageAdded { get; } = new Package.Event();
		public Package.Event onPackageRemoved { get; } = new Package.Event();
		...
		private void RefreshExtensionAreas() {
			_widthExtensionArea.RefreshOnWidth(_width, _length, 2 * _length);
			_lengthExtensionArea.RefreshOnLength(_width, _length, 2 * _width);
		}

		public void SetExtensionAreasVisible(bool visible) {
			_widthExtensionArea.gameObject.SetActive(visible && _width < _maxWidth && (GameSessionData.current?.credits ?? 0) >= _widthExtensionArea.cost);
			_lengthExtensionArea.gameObject.SetActive(visible && _length < _maxLength && (GameSessionData.current?.credits ?? 0) >= _lengthExtensionArea.cost);
		}
		...
		public IEnumerable<Package> GetPackagesReadyToDeliver() => packagesReadyToDeliver;
	}
}